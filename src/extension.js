...
function activate(context) {
	this.extensionName = 'AdrixCorp.technoir-vscode';
	this.cntx = context;
	
	const config = vscode.workspace.getConfiguration("technoir");
	...
	let disposable = vscode.commands.registerCommand('technoir.enableNeon', function () {
		...
		try {
			...
			if (!isEnabled) {
				// delete technoir script tag if there
				let output = html.replace(/^.*(<!-- technoir 84 --><script src="neondreams.js"><\/script><!-- NEON DREAMS -->).*\n?/mg, '');
				// add script tag
				output = html.replace(/\<\/html\>/g, `	<!-- technoir 84 --><script src="neondreams.js"></script><!-- NEON DREAMS -->\n`);
				output += '</html>';
	...
	let disable = vscode.commands.registerCommand('technoir.disableNeon', uninstall);
